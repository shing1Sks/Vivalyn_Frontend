...
export default function LanguageVoiceSelector({ onContinue }: Props) {
  ...
  async function handlePlay(pref: string, voiceName: string) {
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
                        {activeLang.voices.map(voice => {
                          ...
                          return (
                            ...
                            >
                              <span className={`text-sm font-medium ${isVoiceActive ? 'text-indigo-700' : 'text-gray-700'}`}>
                                {getVoiceName(selectedLang, voice.preference)}
                              </span>
                              <span className="text-xs text-gray-400 truncate font-mono" title={voice.voice_name}>
                                {voice.voice_name}
                              </span>
                              {/* Play / Loading / Stop button */}
                              <button
                                onClick={e => { e.stopPropagation(); handlePlay(voice.preference, voice.voice_name) }}
